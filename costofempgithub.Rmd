...
```
##setting up model to see if change in prevalence from start of year to end of year is associated with spending majority of investment on corrective actions
```{r}
...
costofemp_redit_final$prevdif<-costofemp_redit_final$Validprev-costofemp_redit_final$Initialprev
correctiveactionmodel3<- lm(costofemp_redit_final$prevdif~costofemp_redit_final$correctivemajority)
summary(correctiveactionmodel3)
```
##identifying if the mean investment into EMP for Groups 1 and 2 is different than Group 3
```{r}
t.test(costofemp_redit_final$EMPinvestmentcost[costofemp_redit_final$esttotalvaluefinished<200001], mu=12613.12)
costofemp_redit_final$EMPinvestmentcost[costofemp_redit_final$esttotalvaluefinished<200001]
```
##identifying correlation between plant's perception of pathogen control and research team's perception of plant's pathogen control. Also identifying correlation between plant's perception of pathogen control and final listeria prevalence
```{r}
cor.test(costofemp_redit_final$perceived_control_past, costofemp_redit_final$research_control_past)
plot(costofemp_redit_final$perceived_control_past, costofemp_redit_final$research_control_past)
